...
app.post('/prods', async (req, res) => {
  ...
  res.json({ id: result.insertId, name:nome, price:preco, description:descricao});
});

app.put('/prods/:id', async (req, res) => {
  const { nome, preco, descricao } = req.body;
  const { id } = req.params;
  await pool.query('UPDATE usuario SET nome = ?, idade = ?, descricao = ? WHERE id = ?', [nome, preco, descricao, id]);
  res.status(200).json({ id: id, name: nome, price: preco, description: descricao });
});
...
//consulta por nome de produto
app.get('/prods/:nome', async (req, res) => {
  try{
    const { nome } = req.params;
    const [rows] =  await pool.query('SELECT id, nome, preco, descricao FROM prodito WHERE nome = ?', [nome]);
